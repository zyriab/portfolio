...
async function fetchRandomBibleVerse() {
    const response =
        await fetch('https://labs.bible.org/api/?passage=random&type=json&callback=amen')
...
function clearSelectionStyling(scrollToTop) {
    if (isMobile()) {
        ...
        const selectedFrameElement = document.getElementsByClassName('selected-frame')[0];
    ...
    if (previousSectionItemIndexElement != null) {
        previousSectionItemIndexElement.innerText =
            `1 of ${previousSection.items.length}`;
    }
...
async function render(scrollToTop = false, isInitialRender = false) {
    if (!isMobile() && !isInitialRender && currentPosition.sectionIndex === previousPosition.sectionIndex
...
function goToNextSection() {
    ...
    currentPosition.sectionIndex =
        clamp(0, currentPosition.sectionIndex + 1, left_sections.length - 1);
...
function goToNextItem() {
    ...
    currentPosition.sectionItemIndex =
        clamp(0, currentPosition.sectionItemIndex + 1,
...
function scrollMainContentDown() {
    MAIN_CONTENT_SECTION?.scrollBy({ top: MAIN_CONTENT_SECTION.clientHeight / 2 });
}
...
const javascriptKeywords = [
    ['colorizeCode', 'getCodeSnippet', 'javascript', 'typescript', 'html',
        'css', 'go', 'c', 'graphql', 'sql', 'react', 'jest', 'bash', 'yaml',
        'arduino', 'innerText', 'classList', '0', '\'code\'', 'document',
        'null', 'innerHTML'],
...
const nodeKeywords = [
    [...javascriptKeywords[0], 'express', 'IS_ONLINE', 'helmet', 'IS_OFFLINE',
        'method', 'graphqlHTTP', 'schema', 'rootValue', 'validationRules',
        'graphiql', 'next'],
...
const cssKeywords = [
    ['footer-item', 'home-section-paragraph', 'container', 'footer',
        'left-section', 'selected-frame', 'container-content'],
    ['display', 'flex-direction', 'justify-content', 'position', 'padding',
        'border-color', 'margin-top', 'font-size', 'flex-direction', 'gap',
        'border', 'overflow-y', 'overflow-x', 'hover', 'not']
];
...
const cKeywords = [
    ['dequeNode', 'data', 'Node', 'createDeque', 'malloc', 'printf', 'exit', 'EXIT_FAILURE', 'NULL', 'front', 'back', 'next', 'prev', 'destroyDeque', 'dequeIsEmpty', 'dequePopFront', 'free', 'memcpy', 'true', 'false', 'dequePushBack', 'dequePushFront', 'dequePopBack', 'dequeFront'],
    ['typedef', 'struct', 'void', 'sizeof', 'if', 'return', 'while', 'bool',
        'else', 'Deque', 'static', 'char', 'const']
];

const graphqlKeywords = [
    ['FileInput', 'FilesInput', 'DirectoryInput', 'ListBucketResult',
        'DeleteFileResult', 'DeleteDirectoryResult', 'Queries',
        'Mutations', 'schema', 'ListInput', 'UploadInput', 'SignedUrlResult',
        'SignedUrlResult', 'TextFileContentResult', 'RestoreFileResult'],
    ['input', 'type', 'union', 'fileName', 'path', 'root', 'versionId',
        'bucketName', 'fileNames', 'versionIds',
        'listBucketContent', 'getUploadUrl', 'getDownloadUrl',
        'getTextFileContent', 'listInput', 'uploadInput', 'fileInput',
        'fileInput', 'filesInput', 'directoryInput',
        'deleteOneFile', 'deleteManyFiles', 'deleteDirectory', 'restoreFileVersion'],
    ['String', 'ObjectList', 'Unauthenticated', 'Unauthorized',
        'StorageNotFound', 'ServerError', 'FileNotFound', 'FileName',
        'FileNameList', 'Directory', 'RestoreFileResult',]
];
...
const reactKeywords = [
    [...typescriptKeywords[0], 'Gallery', 'length', 'imagesUrls', 'false',
        'false', 'useState', '1', 'setShowFullSizeImage'],
...
const yamlKeywords = [
    ['name', 'on', 'push', 'jobs', 'runs-on', 'steps', 'uses', 'with',
        'branches', 'main', 'deploy_lambda', 'ubuntu-latest', 'env', 'run']
];

const arduinoKeywords = [
    [...cKeywords[0], 'setup', 'loop', 'stop', 'go', 'pinMode', 'OUTPUT', 'INPUT', 'HIGH', 'LOW', '58.2', '1000', 'delay', '50', '1000000', 'pulseIn', 'digitalWrite', 'delayMicroSeconds', '10',],
